...
function plot_refined_safety(d, ~, outDir)
    ...
    patch(ax, [0 80 80 0], [9.65 9.65 10.35 10.35], ...
    ...
    text(ax, 1.0, 10.75, 'Safety limit $d_{\rm avoid}=10$ m', ...
        'Color', [0.82 0.16 0.19], 'FontSize', 8.0, ...
        'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom');
    plot(ax, d.switchTime, 60.5, 'v', 'Color', [0.16 0.18 0.20], ...
        'MarkerFaceColor', [0.16 0.18 0.20], 'MarkerSize', 5.5, ...
        'HandleVisibility', 'off');
    text(ax, d.switchTime + 1.0, 59.7, ...
        '$t=50$ s: USV5, USV6 misbehaved', 'FontSize', 8.2, ...
        'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');
    xlabel(ax, 'Time (s)');
    ylabel(ax, 'Inter-USV distance (m)');
    xlim(ax, [0 80]);
    ylim(ax, [8 62]);
    legendOrder = reshape(reshape(1:numel(hPairs), 3, 5).', 1, []);
    lgd = legend(ax, hPairs(legendOrder), ...
        'Orientation', 'horizontal', 'NumColumns', 5, ...
        'Box', 'on', 'FontSize', 7.2, 'Color', [1 1 1], ...
        'EdgeColor', [0.38 0.40 0.42], 'LineWidth', 0.65);
    lgd.ItemTokenSize = [19 7];
    lgd.Layout.Tile = 'south';
